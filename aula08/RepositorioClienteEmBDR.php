...
class RepositorioClienteEmBDR implements RepositorioCliente {
    ...
    function adicionar( Cliente &$c ) {
        ...
        try {
            ...
            $ps = $this->pdo->prepare( 'INSERT INTO cliente_telefone ( numero, cliente_id )
                VALUES ( :numero, :cliente_id )' );
            foreach ( $c->telefones as $tel ) {
                $ps->execute( [ 'numero' => $tel->numero, 'cliente_id' => $c->id ] );
            }
    ...
    function removerPeloId( $id ) {
        try {
            $ps = $this->pdo->prepare( 'DELETE FROM cliente WHERE id = ?' );
            $ps->execute( [ $id ] );
            if ( $ps->rowCount() < 1 ) {
    ...
    private function telefoneExiste( $numero ) {
        try {
            $ps = $this->pdo->prepare( 'SELECT id FROM cliente_telefone WHERE numero = ?' );
            $ps->execute( [ $numero ] );
    ...
    private function telefonesDoClienteComId( $idCliente ) {
        try {
            $ps = $this->pdo->prepare( 'SELECT id, numero FROM cliente_telefone WHERE cliente_id = ?' );
            $ps->execute( [ $idCliente ] );
            $registros = $ps->fetchAll();
            $telefones = [];
            foreach ( $registros as $r ) {
                $telefones []= new Telefone( $r[ 'id' ], $r[ 'numero' ] );
            }
            return $telefones;
        } catch ( PDOException $e ) {
            throw new RepositorioException( 'Erro ao consultar os clientes.' );
